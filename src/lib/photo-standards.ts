...
/** Group standards by category for UI rendering. */
export function groupedStandards(): Record<string, PhotoStandard[]> {
  ...
  for (const s of PHOTO_STANDARDS) {
    const key = s.flag.includes("🇮🇳") ? "🇮🇳 India"
              : s.id === "icao"         ? "🌐 International"
              : s.flag.includes("🇺🇸") || s.flag.includes("🇨🇦") ? "🌎 Americas"
              : s.flag.includes("🇬🇧") || s.flag.includes("🇪🇺") || s.flag.includes("🇩🇪") ? "🇪🇺 Europe"
              : s.flag.includes("🇦🇪") || s.flag.includes("🇸🇦") ? "🌍 Middle East"
              : "🌏 Asia-Pacific";
    groups[key] ??= [];
    groups[key].push(s);
  }
